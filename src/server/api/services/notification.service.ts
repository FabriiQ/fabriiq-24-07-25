...
export class NotificationService extends ServiceBase {
  ...
  async createNotification(data: z.infer<typeof createNotificationSchema>) {
    try {
      ...
      // Create a real notification using the existing User model's metadata field
      // We'll store notifications in a JSON format in user metadata
    ...
    try {
      ...
      if (query.isRead !== undefined) {
        filteredNotifications = notifications.filter((n: any) => {
          // Ensure notification object has required properties
          return n && typeof n === 'object' && typeof n.isRead === 'boolean' && n.isRead === query.isRead;
        });
      }
      ...
      // Transform to expected format with proper validation
      const transformedNotifications = limitedNotifications
        ...
        .map((notification: any) => ({
          ...
          sender: {
            id: notification.senderId || 'system',
      ...
      return {
        ...
        nextCursor: filteredNotifications.length > limit ? "next_cursor" : undefined,
      };
    } catch (error) {
      console.error('Error in getUserNotifications:', error);

      // Return empty notifications instead of throwing error to prevent UI crashes
      return {
        success: true,
        notifications: [],
