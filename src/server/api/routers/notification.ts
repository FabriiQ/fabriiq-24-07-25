...
export const notificationRouter = createTRPCRouter({
  ...
  markAsRead: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const service = new NotificationService({ prisma: ctx.prisma });
      return service.markNotificationAsRead(input, ctx.session.user.id);
    }),
  ...
  markAllAsRead: protectedProcedure
    .mutation(async ({ ctx }) => { // Remove input parameter as it's not needed
      ...
      const service = new NotificationService({ prisma: ctx.prisma });
      return service.markAllNotificationsAsRead(ctx.session.user.id);
    }),
  ...
  getStats: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const service = new NotificationService({ prisma: ctx.prisma });
      return service.getUnreadNotificationCount(input || ctx.session.user.id);
    }),
